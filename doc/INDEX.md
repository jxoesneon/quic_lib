# dart_quic Documentation Index

## Architecture

- [API Surface Architecture](architecture/API_SURFACE.md)
- [Crypto Backend Abstraction](architecture/CRYPTO_ABSTRACTION.md)
- [DART-IPFS Integration Contract](architecture/DART_IPFS_INTEGRATION.md)
- [Data Flow](architecture/DATA_FLOW.md)
- [Module Overview](architecture/MODULE_OVERVIEW.md)
- [Roadmap Architecture](architecture/ROADMAP_ARCHITECTURE.md)

## Research

- [Dart Ecosystem Gap Analysis](research/DART_ECOSYSTEM_GAP.md)
- [libp2p QUIC Spec Notes](research/LIBP2P_QUIC_SPEC_NOTES.md)
- [Prior Art Analysis](research/PRIOR_ART_ANALYSIS.md)
- [RFC 9000 Notes](research/RFC_9000_NOTES.md)
- [RFC 9001 Notes](research/RFC_9001_NOTES.md)
- [RFC 9002 Notes](research/RFC_9002_NOTES.md)
- [RFC 9114 Notes](research/RFC_9114_NOTES.md)
- [RFC 9204 Notes](research/RFC_9204_NOTES.md)
- [WebTransport Draft Notes](research/WEBTRANSPORT_DRAFT_NOTES.md)

## Specifications

- [CUBIC Spec](specs/CUBIC_SPEC.md)
- [Dart API Spec](specs/DART_API_SPEC.md)
- [DCUTR Spec](specs/DCUTR_SPEC.md)
- [Error Registry](specs/ERROR_REGISTRY.md)
- [Fuzzing Spec](specs/FUZZING_SPEC.md)
- [HTTP/3 Spec](specs/HTTP3_SPEC.md)
- [libp2p QUIC Spec](specs/LIBP2P_QUIC_SPEC.md)
- [Performance Benchmarking](specs/PERFORMANCE_BENCHMARKING.md)
- [QUIC Crypto Spec](specs/QUIC_CRYPTO_SPEC.md)
- [QUIC Datagram Spec](specs/QUIC_DATAGRAM_SPEC.md)
- [QUIC Recovery Spec](specs/QUIC_RECOVERY_SPEC.md)
- [QUIC Streams Spec](specs/QUIC_STREAMS_SPEC.md)
- [QPACK Spec](specs/QPACK_SPEC.md)
- [QUIC Transport Parameters Spec](specs/QUIC_TRANSPORT_PARAMETERS_SPEC.md)
- [QUIC Wire Spec](specs/QUIC_WIRE_SPEC.md)
- [Roadmap](specs/ROADMAP.md)
- [Security Spec](specs/SECURITY_SPEC.md)
- [Testing Spec](specs/TESTING_SPEC.md)
- [Test Vectors](specs/TEST_VECTORS.md)
- [Versioning Policy](specs/VERSIONING_POLICY.md)
- [WebTransport Spec](specs/WEBTRANSPORT_SPEC.md)

## Decisions

- [ADR-001: Pure Dart — No FFI Dependencies](decisions/ADR-001_Pure_Dart_No_FFI.md)
- [ADR-002: NewReno Before CUBIC](decisions/ADR-002_NewReno_Before_CUBIC.md)
- [ADR-003: Soft-Fail Certificate Revocation](decisions/ADR-003_Soft_Fail_Certificate_Revocation.md)
- [ADR-004: package:cryptography as Primary Crypto Backend](decisions/ADR-004_Cryptography_Primary_Crypto_Backend.md)
- [ADR-005: Three-Tier Documentation](decisions/ADR-005_Three_Tier_Documentation.md)
- [ADR-006: Stream Scheduler as Pluggable Interface](decisions/ADR-006_Stream_Scheduler_Pluggable_Interface.md)
- [ADR-007: Isolate-per-Connection Architecture](decisions/ADR-007_Isolate_per_Connection_Architecture.md)

## Contributing

- [Extension & Contribution Guide](EXTENSION_GUIDE.md)
