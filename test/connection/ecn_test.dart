...
void main() {
  ...
  group('ECN transport parameter', () {
    test('includes ecn parameter when ecnEnabled is true', () {
      final conn = _createConnection(ecnEnabled: true);
      final tp = conn.buildTransportParameters();
      expect(tp, contains(QuicTransportParameterId.ecn.value));
    });

    test('excludes ecn parameter when ecnEnabled is false', () {
      final conn = _createConnection(ecnEnabled: false);
      final tp = conn.buildTransportParameters();
      expect(tp, isNot(contains(QuicTransportParameterId.ecn.value)));
    });
  });
}
