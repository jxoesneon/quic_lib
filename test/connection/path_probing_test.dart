...
void main() {
  group('QuicConnection path probing', () {
    ...
    test('isProbingPath is true after probeNewPath', () async {
      ...
      final challengeFrame =
          result!.frames.whereType<PathChallengeFrame>().first;
      final responseDatagram = await _buildResponseDatagram(
          challengeFrame.data, List<int>.filled(8, 0));
      conn.processIncomingDatagram(responseDatagram);
      await future;
    });
    ...
    test('Packet contains a PathChallengeFrame', () async {
      ...
      // Clean up
      final responseDatagram =
          await _buildResponseDatagram(challengeFrames.first.data, dcid);
      conn.processIncomingDatagram(responseDatagram);
      await future;
    });
  });
